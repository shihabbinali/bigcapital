...
- **tsconfig split (required)**: main `packages/server/tsconfig.json` uses `module: esnext` + `verbatimModuleSyntax: true` for bun. ts-jest compiles to CJS so it MUST use `tsconfig.spec.json` / `test/tsconfig.spec.json` (`module: commonjs`, `verbatimModuleSyntax: false`, `esModuleInterop: true`). Wired via `jest.globals['ts-jest'].tsconfig` and `test/jest-e2e.json` `globals`. All 56 e2e specs use `import request from 'supertest'` (default import, not `import * as`).
